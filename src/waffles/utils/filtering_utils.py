...
def percentileFilter(
    ...
    percentileConditonUpper = parameterValues > np.min(percentileValues)
    percentileConditonLower = parameterValues < np.max(percentileValues)
    percentileConditon = percentileConditonLower & percentileConditonUpper
...
def coarse_selection_for_led_calibration(
    ...
    if np.max(waveform.adcs) > upper_threshold or np.min(waveform.adcs) < lower_threshold:
        return False
