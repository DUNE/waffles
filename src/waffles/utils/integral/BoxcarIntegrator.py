...
from waffles.utils.numerical_utils import applyDiscreteFilter

class BoxcarIntegrator(WfAna):
    ...
    def __init__(self, input_parameters: IPDict):
        ...
        self.__window_length = input_parameters['window_length']
    ...
    @property
    def avg_window_length(self):
        return self.__avg_window_length
    ...
    @property
    def max_value_avg(self):
        return self.__max_value_avg
    ...
    def analyse(self, waveform: WaveformAdcs) -> None:
        ...
        None
        """
        is_inverted_waveform = ((1. if not self.__inversion else -1.)/self.__max_value_avg) * (waveform.adcs)
        
        filtered_adcs = applyDiscreteFilter(
            is_inverted_waveform,
            filterType = 'Boxcar',
            boxcarFilterLength = self.__window_length
            )
        
        # Average around the center of the maximum
        integral = np.trapz(
            y=filtered_adcs[self.__max_pos - self.__avg_window_length : self.__max_pos + self.__avg_window_length],
            dx=waveform.time_step_ns
        )
