...
@lru_cache(maxsize=1)
def get_cb_march26_channel_mapping() -> pd.DataFrame:
    """
    This function reads the Coldbox-March channel mapping file and returns it as a pandas DataFrame.
    The dataframe contains the following columns:
    ConfigCh, DAQCh, Module, SiPM, LN2Vbr
    Parameters
    ----------
    ...
    """
    wafflesdir = Path(waffles.__file__).parent
    mapping_file = wafflesdir / "coldboxVD" / "utils" / "CB_March26_ChannelMap.csv"
    if not Path(mapping_file).exists() :
        print(mapping_file)
        raise FileNotFoundError(
            "The channel mapping was not found. You probably need to install waffles with -e option:\n`python3 -m pip install -e .`")
