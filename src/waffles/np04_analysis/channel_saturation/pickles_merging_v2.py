# # # TO DO:
# magari diamo opzione per selezionare solo un run?? magari poi
# dare la possibilità di selezionare un CHANNEL (anora da implementare la lista)


'''
Some examples:

- merging_mode 'from-to'
python pickles_merging_v2.py --output_filename pippo --set_name A --merging_mode 'from-to' --index_start 2 --index_stop 15

- merging_mode 'list_index'
python pickles_merging_v2.py --output_filename pippo --set_name A --merging_mode 'list_index' --index_list '2,5,9,6,3,16'

- merging_mode 'readNfiles'
python pickles_merging_v2.py --output_filename pippo --set_name A --merging_mode 'readNfiles' --n_files 6
...
def main(full_streaming, pickles_folder, output_filename, overwrite, merging_mode, n_files, index_list, index_start, index_stop, beam_data, beam_selection, timeoffset_min, timeoffset_max, set_name, apa_list, endpoint_list, save_file):
    ...
    for run in [d for d in os.listdir(pickles_folder) if os.path.isdir(os.path.join(pickles_folder, d))]:  
        print(f"\n\n--- Reading run {run.split('_')[-1]} ---")
        run_folder = os.path.join(pickles_folder, run)
        if os.path.isdir(run_folder):
            os.chdir(run_folder)  
            all_files_indices = sorted([int(re.search(r'\d+', f).group()) for f in [f for f in os.listdir() if f.endswith('.pkl') and (("full" in f) if full_streaming else ("self" in f))]])
