        ...
        for ch_pairs in adiacent_channels_apa:
            ...
            if n_valid <= 3:
                ...
                if p2_evaluation:
                    ...
                    # A = 1 / (snr1 * all_mean_pe_1)
                    # B = 1 / (snr2 * all_mean_pe_2)
                    # y = np.sqrt(A**2 + B**2)
                    # term1 = (A**2 / y)**2 * ((snr1_err/snr1)**2 + (all_mean_pe_1_err/all_mean_pe_1)**2)
                    # term2 = (B**2 / y)**2 * ((snr2_err/snr2)**2 + (all_mean_pe_2_err/all_mean_pe_2)**2)
                    # ey = np.sqrt(term1 + term2)
                    # label_p2 = r'$\sqrt{\left(\frac{1}{SNR_1 \cdot \langle N_{PE,1} \rangle}\right)^2 + \left(\frac{1}{SNR_2 \cdot \langle N_{PE,2} \rangle}\right)^2}$'
                    ...
                    ey = np.sqrt(eA + eB) / y
                    label_p2 = r'$\sqrt{\left(\frac{\sigma_{0,1}/gain_1}{\langle N_{PE,1} \rangle}\right)^2 + \left(\frac{\sigma_{0,2}/gain_2}{\langle N_{PE,2} \rangle}\right)^2}$'
