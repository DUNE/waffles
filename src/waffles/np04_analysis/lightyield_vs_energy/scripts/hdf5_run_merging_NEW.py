...
@click.option("--trigger",
            type=click.Choice(["full_streaming", "self_trigger"], case_sensitive=False),
            required=True,
            help="Trigger mode: 'full_streaming' or 'self_trigger'.")
@click.option("--event_type",
            type=click.Choice(["beam_only", "no_selection"], case_sensitive=False),
            required=True,
            help="Event type to process: 'beam_only' or 'no_selection'.")
@click.option("--input_folder",
              default = "/afs/cern.ch/work/a/anbalbon/public/reading_rucio", #"/afs/cern.ch/work/a/anbalbon/public/reading_beamrun_NEW", # /eos/user/a/anbalbon/reading_beamrun_NEW
              help="Input folder (where processed hdf5 files are, without run number)")
@click.option("--output_folder",
              default = "/afs/cern.ch/work/a/anbalbon/public/reading_rucio",
...
def main(run_number, trigger, event_type, input_folder, output_folder, output_name, overwrite, save_file):

    run_folder = os.path.join(input_folder,f"run0{run_number}",trigger, event_type)
    if not os.path.isdir(run_folder):
        raise click.BadParameter(f"The folder {run_folder} does not exist")

    # Output file
    output_filename = f"{output_folder}/run0{run_number}/run0{run_number}_{trigger}_{event_type}_{output_name}.pkl"
