...
@click.option("--input_folder",
              default = "/eos/user/a/anbalbon/reading_beamrun_NEW", #"/afs/cern.ch/work/a/anbalbon/public/reading_beamrun_NEW",
              help="Input folder (where processed hdf5 files are, without run number)")
@click.option("--output_folder",
              default = "/eos/user/a/anbalbon/reading_beamrun_NEW",
...
def main(run_number, input_folder, output_folder, output_name, overwrite, save_file):
    ...
    # Output file
    output_filename = f"{output_folder}/run0{run_number}/{output_name}.pkl"
    ...
    if save_file == "yes" and i_index != 0:
        ...
        summary_txt = os.path.join(f"{output_folder}/run0{run_number}", "summary.txt")
        new_summary_entry = (
