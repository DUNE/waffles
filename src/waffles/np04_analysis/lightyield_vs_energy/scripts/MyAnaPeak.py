# Analysis class to search for peaks
...
class MyAnaPeak(WfAna):
    ...
    def analyse(self, waveform: WaveformAdcs) -> None:
        ...
        if self.__baseline_method == "EasyMedian":
            ...
            self.__baseliner.baselinestart -= waveform.time_offset
            self.__baseliner.baselinefinish -= waveform.time_offset
        ...
        ####  peak finding 
        std = np.std(-waveform.adcs)
        mean = np.mean(-waveform.adcs)
        ...
        peak_amplitude = properties['peak_heights']
        ...
        mask = (time_offset >= time_offset_min) & (time_offset <= time_offset_max)
