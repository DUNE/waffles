...
def main(config):
    try:
        ...
        # Get run_ch_disctionary where for each run you have
        # a list of 2-item lists
        run_ch_dict = config_data.get("run_ch_dict", {})

        required_keys = ["run_ch_dict", "rucio_dir", "output_dir", "ch"]
        missing = [key for key in required_keys if key not in config_data]
        if missing:
            raise ValueError(f"Missing keys in config: {missing}")

        for run, ch_pairs in run_ch_dict.items():
