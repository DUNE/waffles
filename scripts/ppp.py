...
class WaveformProcessor:
    ...
    def __init__(self, config: dict, run: int):
        ...
        self.beam_selection = config.get("beam_selection", "")
        self.remove_rucio_file = config.get("remove_rucio_file", False)   # ✅ nuovo parametro
    ...
    def read_and_save(self):
        ...
        try:
            ...
            filtered_filepaths = []
            run_dir = Path(self.output_path) / f"run{self.run_number:06d}{f'_{self.suffix}' if self.suffix else ''}"
            for filepath in filepaths:
                input_name = Path(filepath).name
                output_name = f"processed_{input_name}_structured{f'_{self.suffix}' if self.suffix else ''}.hdf5"
                output_path = run_dir / output_name

                if not output_path.exists():
                    filtered_filepaths.append(filepath)
                else:
                    print_colored(f"⚠️ Skipping already processed file: {output_path}", color="WARNING")
            filepaths = filtered_filepaths

            if not filepaths:
                print_colored("✅ Tutti i file risultano già processati. Nulla da fare.", color="INFO")
                return True
            ...
            if self.save_single_file:
                ...
                if self.beam_selection:
                    try:
                        self.wfset = WaveformSet.from_filtered_WaveformSet(self.wfset, beam_self_trigger_filter, timeoffset_min=-120, timeoffset_max=-90)
                    except Exception as e:
                        if "There must be at least one Waveform in the set" in str(e):
                            self.wfset = False
                            print('NO BEAM WAVEFORMS')
                        else:
                            self.wfset = False
                            raise 
                ...
                for file in filepaths:
                    ...
                    if self.beam_selection:
                        try:
                            wfset = WaveformSet.from_filtered_WaveformSet( wfset, beam_self_trigger_filter, timeoffset_min=-120, timeoffset_max=-90)
                        except Exception as e:
                            if "There must be at least one Waveform in the set" in str(e):
                                wfset = False
                                print('NO BEAM WAVEFORMS')
    ...
    def write_output(self, wfset, input_filepath):
        ...
        try:
            ...
            # ✅ elimina file originale se richiesto
            if self.remove_rucio_file:
                try:
                    Path(input_filepath).unlink()
                    print_colored(f"🗑️ Removed original file: {input_filepath}", color="WARNING")
                except Exception as e:
                    print_colored(f"⚠️ Could not remove {input_filepath}: {e}", color="ERROR")
